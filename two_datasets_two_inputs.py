...
from data_generation_utils import (
    ...
    apply_random_control_symmetric
)
...
def generate_initial_conditions(n_trajectories, model, data, qpos_init, qvel_init,
    ...
    for traj in range(n_trajectories):
        ...
        if traj < random_switch_index:
            u_init = apply_random_control_symmetric(data, model, perturbation_duration,
                                          control_scale_random, update_frequency)
        else:
            u_init = apply_random_control_symmetric(data, model, perturbation_duration,
                                          control_scale_random_2, update_frequency)
        # --- Enforce symmetry ---
        # The original u_init is generated as a 4-element vector.
        # We now define the independent inputs as the first and third elements.
        # Later in the actuator simulation these two values will be expanded to:
        # [u[0], -u[0], u[1], -u[1]]
        initial_conditions.append((data.qpos.copy(), data.qvel.copy()))
        initial_controls.append(u_init)  # u_init is the half dimensional inputs for the symmetric case
...
def simulate_actuator(initial_condition, u_init, model, data, duration,
    ...
    while data.time < duration and not converged:
        ...
        # Here u_init is now a 2-element vector (the independent controls).
        raw_control = get_new_lambda_u(data.time, lambda_matrix, u_init)
        # Enforce symmetry: the applied control has four channels:
        # axis 1: raw_control[0] and its negative; axis 2: raw_control[1] and its negative.
        ...
        if include_velocity:
            state += ee_velocity.tolist() + np.hstack(geom_velocities).tolist()
        state += raw_control.tolist()  # current_control.tolist()  # Append the applied control values.
        trajectories.append(state)
...
def main():
    ...
    n_trajectories = 120
    n_trajectories_random_switch_to_2 = 60
    update_frequency = 50
    ...
    control_scale_random = 0.6
    control_scale_random_2 = 0.4
    ...
    # Now that the model uses only two independent inputs, we use two eigenvalues.
    eigenvalues = [-2, -2.5]
    ...
    data_folder_perfect = os.path.abspath(os.path.join(current_dir, "..", "trajectories_b_test", "data_larger_no_lambda"))
    video_folder_perfect = os.path.abspath(os.path.join(current_dir, "..", "trajectories_b_test", "videos_larger_no_lambda"))
    data_folder_actuator = os.path.abspath(os.path.join(current_dir, "..", "trajectories_b_test", "data_larger_with_lambda"))
    video_folder_actuator = os.path.abspath(os.path.join(current_dir, "..", "trajectories_b_test", "videos_larger_with_lambda"))
    ...
    # Although the independent inputs are 2D, the applied control is expanded to 4D.
