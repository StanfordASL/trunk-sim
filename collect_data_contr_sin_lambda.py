...
control_scale = 0.002  # Scale for control inputs
...
# Output folder setup
folder_path = os.path.join(current_dir, 'trajectories', 'data_controlled')
...
for traj_idx in range(num_trajectories):
    ...
    current_real_control = np.zeros(num_controls)
    ...
    with mujoco.Renderer(model) as renderer:
        ...
        while data.time < duration:
            ...
            time = data.time
            #current_real_control = compute_u_dynamics_sin(current_real_control, last_time, time, lambda_matrix, offsets, amplitudes, frequencies)
            current_real_control = compute_u_step(step_moment, time, step_control_values, lambda_matrix, num_controls)
            #current_desired_control = offsets + amplitudes * np.sin(frequencies * time)

            if time < step_moment:
                current_desired_control = np.zeros(num_controls)
            else:
                current_desired_control = step_control_values

            data.ctrl[:] = current_real_control
            last_time = time
    ...
    #    trajectory[i][1:] = (np.array(trajectory[i][1:]) - steady_state_position_slow_osc[1:]).tolist()
    ...
    control_columns_desired = [f"control_desired_{i}" for i in range(model.nu)]
    ...
    # Save rendered video
    if create_video:
        video_folder = os.path.join(current_dir, "trajectories", "videos_controlled")
